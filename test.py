...
class Ball:
    ...
    def move(self):
        ...
        if self.y >= 467:
            if(self.check_collision_with_board(board)):
                self.vy = -self.vy
                self.vx=1.05*abs(self.vy)*(ball.x-board.x-37.5)/37.5
            else:
    ...
    def check_collision_with_board(self, board):
        if self.x >= board.x-4 and self.x <= board.x + 79 and self.y >= 467 and self.y <= 480:
            return True
        return False
...
class Board:
    ...
    def draw(self):
        pygame.draw.rect(screen, (255, 0, 0), (self.x, 465, 75, 15), 0)
        pygame.draw.rect(screen, (0, 225, 225), (self.x, 460,5,5), 0)
        pygame.draw.rect(screen, (0, 225, 225), (self.x+70, 460,5,5), 0)
...
def main():
    ...
    while time>0:
        ...
        if keys[pygame.K_LEFT]:
            ...
            bullets.add_bullet(board.x+75, 465)
            cool_down=10
        ball.update()
        blocks.update(ball)
        bullets.update()
